...
The key function to use for finding the power and sample size (PASS) for the test is `fPASS::PASS_Proj_Test_ufDA()`. In the following, we will see how the function can be used to conduct PASS for the Projection-based test for longitudinal and functional data under different covariance structure in order to design a randomized clinical trials. We will present the examples of using the function with different case studies. 

### Case study 1: Longitudinal data with a specific visit times. 
...
required_sample_size <- foreach(i=1:20, .combine='c', .packages = "fPASS") %dopar% {
        ...
        PASS_Proj_Test_ufDA(sample_size = NULL, target.power = target.power,
                            ...
                            fpca_optns = fpca_optns)$required_SS
}
stopCluster(cl)
quantile(required_sample_size, probs = c(0,0.05,0.1,0.25,0.5,0.75,0.9,0.95,1), names = TRUE)
```
...
cl <- makeCluster(8)
registerDoParallel(cl)
required_sample_size <- foreach(i=1:50, .combine='c', .packages = "fPASS") %dopar% {
        mean_diff_fn <- mean_diff_fn
        PASS_Proj_Test_ufDA(sample_size = NULL, target.power = target.power,
                            sig.level = sig.level, nobs_per_subj = nobs_per_subj,
                            obs.design = obs.design, mean_diff_fnm = mean_diff_fnm,
...
cl <- makeCluster(8)
registerDoParallel(cl)
required_sample_size <- foreach(i=1:100, .combine='c', .packages = "fPASS") %dopar% {
        mean_diff_fn <- mean_diff_fn
        PASS_Proj_Test_ufDA(sample_size = NULL, target.power = target.power,
                            sig.level = sig.level, nobs_per_subj = nobs_per_subj,
                            obs.design = obs.design, mean_diff_fnm = "mean_diff_fn",
